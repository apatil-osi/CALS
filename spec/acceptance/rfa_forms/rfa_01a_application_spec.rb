...
RSpec.feature 'RFA01A', js: true do
  ...
  scenario 'validate full applicant card', set_auth_header: true do
    ...
    fill_in('first_name', with: Faker::Name.first_name, : match => : prefer_exact)# fill_in('middle_name', with: 'k', : match => : prefer_exact)# fill_in('last_name', with: Faker::Name.last_name, : match => : prefer_exact)
  ...
  scenario 'validate submit button functionality', set_auth_header: true do
    ...
    click_button 'Submit'#
    DevNote: this test
    case will be updated to include more fields as submit# functionality is further fleshed out.
    end

    scenario 'show error validation message on full Applicant Card', set_auth_header:
    true do
      visit root_path
      click_button 'Create RFA Application (Form 01)'
      expect(page).to have_content 'Rfa-01A Section Summary'
    ...
    scenario 'remove error validation on full Applicant card', set_auth_header: true do
      ...
      click_button('Save Progress')# expect(page).to have_content "Message: Applicant with first name - [Geovanni], last name - [Moen] and name suffix - [] already exists in application"
      click_button('Add Another Applicant +')
      fill_in('applicants[1].first_name', with: Faker::Name.first_name, match: : prefer_exact)
      fill_in('applicants[1].last_name', with: Faker::Name.last_name, match: : prefer_exact)
      click_button('Save Progress')
    ...
    # scenario 'remove leading spaces in the names', set_auth_header: true do# visit root_path# click_button 'Create RFA Application (Form 01)'#
    expect(page).
      to have_content 'Rfa-01A Section Summary'#
    page.find('#Rfa01AOverview').find('a.btn.btn-default').click# expect(page).to have_content 'Applicant 1 - Information'#
    fill_in 'applicants[0].first_name', with: " "#
    fill_in 'applicants[0].last_name', with: " "#
    click_button('Save Progress')# expect(find_field('applicants[0].first_name').value).to eq 'first'#
